...
//!
//! 自 `skill_stat_map.rs`**原样平移**的两个
//! 存留映射：它们服务于 orchestrator 的光环防御 buff（`buff_skill_specs` 构造
//! BuffSpec.mods——切换后旧 `aura_buff_modifiers` 静态直注已删，本映射
...
//! 退役（声明：BuffSpec 框架先行，buff stat→mod 数据化映射 defer——C5-3
//! 删旧码时本映射因仍被 `buff_skill_specs` 消费而**保留**，范围内）。
//!
//! **退役评估（2026-06 实查 `skill_stat_map.json` + vendor 数据）——本波不切**：
//!
...
//!    effectType=Aura 过滤 + player 侧允收名单翻译 + tag 直译）——属声明
...
pub fn map_aura_buff_stat(stat: &str) -> Vec<MappedStat> {
    ...
    match stat {
        ...
        // 该 stat 不计算，实查）。旧映射在 additional-granted-effects 展开
