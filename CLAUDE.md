...
- Equipment, skill and passive planners share a persisted goal. Passive planning uses connected routes and path unions including travel cost, and bounded leaf-branch refunds with paths recomputed after refund. Preserve `unlock_constraint` from game tree data (ascendancy and prerequisite nodes), protect its prerequisites and filled sockets, and reject new unsupported effects. New travel-attribute choices are evaluated and applied atomically with nodes. Weapon-exclusive/unknown/disconnected imports disable automatic refunds.
...
- Shared page headers, card/control tokens and slot symbols live under `web/src/components/shared` and `web/src/styles`. UI verification covers all eight tabs and 320/768/1024/1440 breakpoints with real WASM.
...
Build references: `web/src/components/guidance/BuildGuidancePanel.tsx` ranks dated public poe.ninja fixtures against the current editable session. `sync-data` also generates the ignored reference catalog. Historical match points are not upgrade percentages; equipment candidates enter the existing replacement checker without applying. See `devs/docs/architecture/21-build-reference-guidance.md` for data provenance, matching rules and live-index follow-up.
