...
- `web/src/lib/tradeExport.ts` supplies a self-contained bookmark for authenticated official tabs. It exports candidate items/prices to a local JSON file without reading cookies/tokens; the file importer checks realm/league/category/gem, official links, current price cap and level requirements before recalculation. Test the production-minified bookmark, browser download and import together in Playwright.
