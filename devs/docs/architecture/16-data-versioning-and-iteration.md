...
> 精确对应 `4.5.2.1.3`，`4.5.0.3.4` 的树是更早快照——gap B「per-build 树版本」的实证。
>
> **但 golden 基线仍应锁 `4.5.0.3.4`——已实测 settle**（把 `GOLDEN_PARITY_DATA_VERSION` 临时
> 翻到 `4.5.2.1.3` 跑 `parity_baseline_report`，聚合命中全指标显著倒退）：
>
> | 聚合命中 | 4.5.0.3.4 | 4.5.2.1.3 |
> |---|---|---|
> | defensive @5% / @10% | 84.2% / 87.1% | 75.6% / 82.2% |
> | core-8 @5% / @10% | 91.7% / 93.1% | 87.5% / 90.3% |
> | offensive @5% / @10% | 76.2% / 86.2% | 60.0% / 71.2% |
> | dot @5% / @10% | 45.9% / 56.8% | 32.4% / 40.5% |
>
> 即 demo build 内嵌的 golden `player_stats` 是 **`4.5.0.3.4`-era PoB2 数据整体**算出的——
> mods/skills 在 `4.5.2.1.3` 被重平衡（见 §4 语义 diff）把 pobr 输出推离 golden，远盖过那 3 个
> cold 节点的局部增益。故树 0_5 的 3 节点缺口只是 `4.5.0.3.4` *树导出*的局部小缺陷（正规修法
> = GGG 0_5 树再导出，云端 CDN 404 不可得；**不**跨版本拷贝节点混版），与「golden 基线选哪版」
> 是两件事——后者数据已判 `4.5.0.3.4`。
