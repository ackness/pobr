...
## Fix #3 (Essence Drain) — LANDED (2026-06-28)

Root cause (confirmed via pob2-oracle on the build): essence drain's DoT statSet
(`EssenceDrainDotPlayer`, vendor index 2 = the build's selected set) **inherits the main
set's `spell_*_base_chaos_damage` 62/115 via the export base-merge** (skills.lua:541-549).
PoBR mapped that inherited per-hit base → `ChaosDamageMin/Max` (a hit), producing a phantom
171.76 hit DPS; PoB2's DoT set deals no hit (oracle `mainOutput.ChaosMinBase = 0`). The DoT
335 comes from a separate stat (`base_chaos_damage_to_deal_per_minute` → `ChaosDot` 179.56),
so the two are cleanly separable.

**Fix:** per-statSet overlay override in `data/<ver>/overlay/skill_stat_map.json`
`per_stat_set.EssenceDrainPlayer["2"]` mapping `spell_minimum/maximum_base_chaos_damage` to
empty `{"mods": []}` → suppresses the inherited hit for the DoT set only; DoT (per-minute)
unaffected. Verified: phantom `ChaosDamageMin/Max` gone, `ChaosDot` 335 intact (ratio 1.0000).

**Result:** essence-drain CombinedDPS 1.51x → 1.00x. offensive 61→62/80 (76.2%→77.5%),
dot 20→22/37 (54.1%→59.5%) @5%, no defensive regression. Baselines raised accordingly.

**⚠️ Maintainability caveat:** `skill_stat_map.json` is generated by `sync-pob-catalog
extract-lua`. This manual `per_stat_set` correction would be lost on regen — the generator
