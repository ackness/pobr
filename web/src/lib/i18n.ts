...
const DICT = {
  ...
  'trade.hint': { 'en-US': 'Choose a goal and budget. We score useful affixes, fetch live listings, and recalculate each complete item on your character.', 'zh-TW': '選目標與預算，自動評估有用詞綴、搜尋在售商品，逐件替換後重算角色提升。', 'zh-CN': '选目标与预算，自动评估有用词条、搜索在售商品，逐件替换后重算角色提升。' },
  ...
  "trade.gemHint": { 'en-US': "Choose a skill group. Compare gem levels, quality and replacement supports, then price the three best candidates. Each result is one purchase with your other gems retained.", 'zh-TW': "選擇技能組，比較等級、品質與替換輔助，搜尋前三種候選的在售價格。每項結果只替換一顆，保留其他寶石。", 'zh-CN': "选择技能组，比较等级、品质与替换辅助，搜索前三种候选的在售价格。每项结果只替换一颗，保留其他宝石。" },
  ...
  'trade.signedIn': { 'en-US': 'Import listings from signed-in trade', 'zh-TW': '從已登入市集匯入商品', 'zh-CN': '从已登录市集导入商品' },
  'trade.exportHint': { 'en-US': 'Drag the export link to your bookmarks bar (or copy its address into a bookmark). Open this trade search and sign in, activate the bookmark there, then import the downloaded JSON here. No cookies or login tokens are exported.', 'zh-TW': '將下方匯出連結拖到書籤列（或複製地址建立書籤）。打開此市集搜尋並登入，在市集頁點該書籤，再將下載的 JSON 匯入此處。不會匯出 Cookie 或登入憑證。', 'zh-CN': '将下方导出链接拖到书签栏（或复制地址建立书签）。打开此市集搜索并登录，在市集页点该书签，再将下载的 JSON 导入此处。不会导出 Cookie 或登录凭证。' },
  'trade.exportBookmark': { 'en-US': 'Export this search to PoBR', 'zh-TW': '將此搜尋匯出至 PoBR', 'zh-CN': '将此搜索导出至 PoBR' },
  'trade.copyBookmark': { 'en-US': 'Copy bookmark address', 'zh-TW': '複製書籤地址', 'zh-CN': '复制书签地址' },
  'trade.importCandidates': { 'en-US': 'Import trade candidates', 'zh-TW': '匯入商品候選', 'zh-CN': '导入商品候选' },
