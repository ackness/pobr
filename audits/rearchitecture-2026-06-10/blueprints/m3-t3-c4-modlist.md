...
| # | 词条族（语料频次） | 不迁移原因 | 候选去向 |
|---|--------------------|-----------|----------|
| R1 | `N% increased Cooldown Recovery Rate`（玩家侧裸形 ×8+×5+…、`for Grenade Skills` ×7、`Bonded:` ×6+×2） | 非敌方向；`CooldownRecovery` 在玩家 db 被 `skill_mechanics.rs::calc_cooldown` 消费（榴弹类 cd-capped DPS），新增解析会改 ninja_parity 进攻值 | 独立行为 commit（M6 parser 规则波次），附 PoB2 ModParser.lua:662 `["cooldown recovery rate"]` nameList 依据 + 逐 build diff |
...
| R3 | `[Attack] Damage Penetrates 15% of Enemy Elemental Resistances`（×2）/ `Damage Penetrates 8% ...`（×1） | 玩家侧穿透词条（vendor `ElementalPenetration`，非 EnemyModifier）；pobr 已有 `ElementalPenetration` 消费链，新增解析会改进攻 parity 值 | 独立行为 commit（M6 parser 规则） |
...
| R5 | `Nearby Enemies have -N% to all Resistances`（语料未命中） | vendor ModParser.lua:4283 产 `ElementalResist`+`ChaosResist`（enemy db 命名），与玩家侧 `*Resistance` 命名不同，需要 enemy 侧名表；递归路径会产错名 | C4 后续小批（带 enemy 名映射） |
